"""Compute the original candidate-conditioned perplexity PPL(S|T)."""
    ...
    with open(output_file, 'w') as save_file:
        with torch.no_grad():
            for idx in tqdm(data_indices, desc=f"GPU {gpu_id} Perplexity Calculation"):
                ...
                context_ids = tokenizer(context_text, return_tensors="pt", truncation=True, max_length=2040)["input_ids"].to(device)
...
def main(args):
    ...
    output_path = args.output_file or "./data_outputs/"+model_n+"/"+args.dataset+"-ppl_ST.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--temporary-dir', default='./data_outputs/tmp/context')
