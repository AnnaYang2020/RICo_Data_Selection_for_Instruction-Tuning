...
#    Modified from the Stanford Alpaca training code for the RICo project.
#
...
def smart_tokenizer_and_embedding_resize(
    ...
    """Resize tokenizer and embedding with a check for output embeddings.

    Note: This avoids issues where output embeddings may not be present in certain models.
    """
    ...
    if num_new_tokens > 0:
        ...
        # Check if the model has output embeddings before proceeding
        output_embeddings = model.get_output_embeddings()
        if output_embeddings is not None:
            output_embeddings = output_embeddings.weight.data
...
def _tokenize_fn(strings: Sequence[str], tokenizer: transformers.PreTrainedTokenizer) -> Dict:
    ...
    labels = [torch.tensor([text[1]]) for text in strings]
    return dict(
        input_ids=input_ids,
        labels=labels,
    )
